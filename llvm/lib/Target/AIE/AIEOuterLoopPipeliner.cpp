...
void AIEOuterLoopPipeliner::remapClonesInto(ArrayRef<Instruction *> Clones,
                                            ValueToValueMapTy &VMap) {
...
SmallVector<Instruction *, 16> AIEOuterLoopPipeliner::cloneAndRemapInsts(
    ArrayRef<Instruction *> Insts, BasicBlock &Dest,
    BasicBlock::iterator InsertPt, ValueToValueMapTy &VMap,
    const Twine &Suffix) {
  SmallVector<Instruction *, 16> Clones;
  for (Instruction *I : Insts)
    Clones.push_back(cloneInstInto(*I, Dest, InsertPt, VMap, Suffix));
  remapClonesInto(Clones, VMap);
  return Clones;
}
...
void AIEOuterLoopPipeliner::cloneInnerLoopIntoCooldown(
  ...
  remapClonesInto(Clones, CoolVMap);
...
bool AIEOuterLoopPipeliner::collectPrologueInstructionsForSplit(
  ...
  LS.PrologueRegion.forEachInstruction([&](Instruction *I) {
    if (LS.isPipelineCandidate(I) && !Part2Set.count(I))
      Out.push_back(I);
  });
