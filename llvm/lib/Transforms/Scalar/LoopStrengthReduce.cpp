...
static bool mayUsePostIncMode(const TargetTransformInfo &TTI,
                              ...
                              ScalarEvolution &SE) {
  // Standard check: Address uses with int/int-vector access type.
  const bool IsAddressUseWithIntAccess =
      (LU.Kind == LSRUse::Address &&
       LU.AccessTy.getType()->isIntOrIntVectorTy());

  // Allow targets to preserve recurrences for Basic uses with specific types.
  // This enables post-increment optimization for targets with non-native
  // pointer sizes (e.g., AIE with 20-bit pointers and i20 GEP indices).
  const bool IsBasicUseWithPreservedType =
      (LU.Kind == LSRUse::Basic && LU.WidestFixupType &&
       TTI.shouldLSRPreserveBasicRecurrence(LU.WidestFixupType));

  if (!IsAddressUseWithIntAccess && !IsBasicUseWithPreservedType)
    return false;
  ...
  if (TTI.isIndexedLoadLegal(TTI.MIM_PostInc, AR->getType()) ||
      TTI.isIndexedStoreLegal(TTI.MIM_PostInc, AR->getType())) {
    const SCEV *LoopStart = AR->getStart();
    // Accept constant starts (e.g., offset 0) or loop-invariant base pointers.
